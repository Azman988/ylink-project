  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans lg:pb-10 pb-20 relative">
      ...
      <button
        ...
        className={`
                    group fixed bottom-8 right-8 z-50 flex items-center justify-center p-4 rounded-full font-bold text-white shadow-2xl 
                    transition-all duration-300 ease-in-out cursor-pointer hover:shadow-xl hover:-translate-y-1 
                    disabled:opacity-70 disabled:cursor-not-allowed disabled:hover:translate-y-0
                    ${activeService === 'solar' ? 'bg-slate-900 shadow-slate-900/30 hover:bg-slate-800' : 'bg-amber-500 shadow-amber-500/30 hover:bg-amber-400'}
                `}
      >
        <ArrowRightLeft
          className={`w-6 h-6 shrink-0 transition-transform duration-500 ease-in-out ${isTransitioning ? 'animate-spin' : 'group-hover:rotate-180'
            }`}
        />
