import { CheckCircle2, Cpu, Laptop, Loader2, Mail, Phone, Smartphone, User, Wrench } from "lucide-react";
import { useEffect, useState } from "react";
import { sendRepairEmail } from "../../../utils/emailService";
...
const RepairLogForm = () => {
    const [formData, setFormData] = useState({
        ...
        deviceCategory: 'laptop', issue: '', honeypot: "", // Bot trap
    });
    const [formStatus, setFormStatus] = useState<'idle' | 'submitting' | 'success'>('idle');

    // Context hooks for toast notifications and user authentication
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        showToast("Submitting your repair request...");

        try {
            await sendRepairEmail(formData);
            showToast("Repair ticket submitted successfully!", "success");
            setFormStatus("success");
        } catch (error) {
            console.error("Repair form submission error:", error);

            // Dynamically retrieves rate limit message or fallback text
            const message = getErrorMessage(
                error,
                "Failed to submit the repair request. Please try again."
            );
    ...
    const groupStyles = {
        input: "w-full px-4 py-3 text-sm text-slate-500 font-mono bg-slate-50 hover:bg-white border border-slate-200 rounded-xl focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 outline-none transition-all",
        ...
        <div className="bg-white rounded-3xl p-8 lg:p-10 border border-slate-200 shadow-xl shadow-slate-200/50 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            {formStatus === 'success' ? (
                <div className="flex flex-col items-center justify-center py-12 text-center animate-fade-in">
                    ...
                    <h2 className="text-xl md:text-2xl font-extrabold text-slate-900 mb-4">Ticket Generated!</h2>
                    <p className="text-slate-600 max-w-md mx-auto mb-8 leading-relaxed">
                        Thank you, {formData.name.split(' ')[0] || 'there'}. Your repair request has been logged and sent to our team for review.
                    </p>
                ...
                <>
                    <div className="mb-8">
                        <h2 className="text-xl font-extrabold text-slate-900 mb-2">Log a Repair Ticket</h2>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="pt-6">
                            <button type="submit" disabled={formStatus === 'submitting'} className={groupStyles.button}>
                                {formStatus === 'submitting' ? <><Loader2 className="w-5 h-5 animate-spin" /> Submitting Ticket...</> : <>Log Repair Ticket <Wrench className="w-5 h-5" /></>}
                            </button>
                        </div>
                    </form>
                </>
