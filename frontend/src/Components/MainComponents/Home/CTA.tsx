...
export default function CTASection() {
    ...
    const nameTokens = useMemo(() => {
        ...
        return {
            firstName: parts !== undefined && parts[0] || 'User',
            initials: (parts !== undefined && parts[0]?.[0] || '') + (parts !== undefined && parts[1]?.[0] || '')
        };
    }, [user?.name]);
                ...
                <div className="bg-slate-950 rounded-3xl p-10 md:p-14 shadow-2xl relative overflow-hidden border border-slate-800">
                    ...
                    {user ? (
                        ...
                        <div className="relative z-10">
                            ...
                            <Link to="/auth?mode=signup" className="w-fit px-8 py-4 bg-white hover:bg-slate-100 text-slate-900 text-sm font-black rounded-xl transition-all shadow-lg shadow-white/10 flex items-center justify-center gap-2 mx-auto cursor-pointer">
                                Create Your Account <ChevronRight className="w-4 h-4" />
                            </Link>
                            <p className="mt-6 text-xs text-slate-500 font-medium">
                                Already have an account? <Link to="/auth" className="text-blue-400 hover:text-blue-300 transition-colors">Sign in here</Link>
                            </p>
                        </div>
                    )}
                </div>
