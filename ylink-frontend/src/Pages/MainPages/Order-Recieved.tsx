...
const OrderReceived: React.FC = () => {
    ...
    const [status, setStatus] = useState<'loading' | 'success' | 'pod_success' | 'error'>('loading');
    ...
    useEffect(() => {
        const verifyOrder = async () => {
            ...
            try {
                ...
                // For Paystack: Your backend should call Paystack's verify API
                // For POD: Your backend just retrieves the order details
                const response = await fetch(`/api/orders/verify?ref=${reference}`);
                ...
                setOrderDetails({
                    orderId: data.orderId || reference,
    ...
    const handleNavToTrack = (orderId: any) => {
        ...
        return (
            <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 selection:bg-blue-500 selection:text-white">
                ...
                <p className="text-slate-500 font-medium text-sm text-center max-w-sm">
                    Please hold on a moment while we securely confirm your transaction with the server.
                </p>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 selection:bg-blue-500 selection:text-white">
                ...
                <p className="text-slate-500 font-medium text-center max-w-md mb-8">
                    We couldn't verify this order reference. If your account was debited, please contact our support team immediately with your reference number.
                </p>
        ...
        <div className="min-h-screen bg-slate-50 py-24 px-4 sm:px-6 flex items-center justify-center selection:bg-blue-500 selection:text-white">
            <div className="max-w-2xl w-full">
                ...
                <div className="text-center mb-10">
                    <div className="w-20 h-20 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg shadow-emerald-500/20 border border-emerald-200 animate-fade-in-down translate-x-1/2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 relative z-10">
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="text-sm font-semibold text-slate-800 capitalize">
                                {orderDetails?.paymentMethod === 'pod' ? 'Pay on Delivery' : orderDetails?.paymentMethod}
                            </p>
                        </div>
                        <div className="space-y-1">
                            <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Total Amount</p>
                            <p className="text-lg font-black text-blue-600">
                                {currency}{orderDetails?.totalAmount.toLocaleString()}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4">
                    {/* Assuming user context is available or you pass the user ID. For now, link to general orders. */}
                    <div 
                        onClick={() => handleNavToTrack(orderDetails?.orderId)} 
